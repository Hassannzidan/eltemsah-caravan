...
export class SpecificationsManager {
  ...
  @Output() addSpecification = new EventEmitter<{
    categoryId: string;
    spec: ProductSpecificationItem;
  }>();
  @Output() updateCategory = new EventEmitter<{ id: string; title: string }>();
  @Output() updateSpecification = new EventEmitter<{
    categoryId: string;
    specId: string;
    key: string;
    value: string;
  ...
  @Output() deleteSpecificationEvent = new EventEmitter<{
    categoryId: string;
    specId: string;
  }>();
  ...
  handleAddCategory() {
    if (
      ...
      this.specifications.some((c) => c.title === this.newCategory.trim())
    )
      return;
  ...
  onAddSpecClick(categoryId: string) {
    console.log('Add Spec Clicked for Category:', categoryId);
    const key = this.getSpecKey(categoryId);
    const value = this.getSpecValue(categoryId);
    if (!key || !value) return;
    const category = this.specifications.find((c) => c.id === categoryId);
    if (
      category?.specifications.some((s) => s.key === key && s.value === value)
    )
      return;

    this.handleAddSpecification(categoryId);
  }
}
